...
class MemoriaTab extends StatefulWidget {
  const MemoriaTab({super.key});

  @override
  State<MemoriaTab> createState() => _MemoriaTabState();
}

class _MemoriaTabState extends State<MemoriaTab> {
  int cantidad = 0;
  late List<MemoriaResponse> historialResponseData;
  @override
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: memoriaResponse(),
  ...
  Future<List<MemoriaResponse>> memoriaResponse() async {
    final String correo = await SecurityStorage.read("email");
    final httpRequest = await http
        .get(Uri.https('fitgame.ga', '/historial/memoria', {'correo': correo}));
